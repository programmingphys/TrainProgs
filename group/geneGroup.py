...
#导入化简模块
def geneNewElement(func_x,generator,unitSet,initSet):
    newSet = []
    ...
    for i in range(len(newSet)):
        #对新生成群元集合中的元素进行化简
        newSet[i] = Simplify(func_x,newSet[i],unitSet)
    newSet = list(set(newSet))
...
def geneGroup(fun_x,generator,unitSet,initSet):
    ...
    stop = False
    while not stop:
        newSet = geneNewElement(fun_x,generator,unitSet,sets[-1])
...
if __name__ == "__main__":
    new = geneNewElement(2,['r','f'],['rrr','ff'],['r','','f'])
